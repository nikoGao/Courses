...
bool AlgebraicTreeExpression::operator==(AlgebraicTreeExpression &src){
    return operatorHelper(this->_root, src._root);
}

bool AlgebraicTreeExpression::operatorHelper(AlgebraicTreeNode *root1, AlgebraicTreeNode *root2){
    if(root1==NULL && root2==NULL) return true;
    else if((root1!=NULL&&root2==NULL)||(root1==NULL&&root2!=NULL)) return false;
    else{
        return root1->operator==(root2);
    }
}
